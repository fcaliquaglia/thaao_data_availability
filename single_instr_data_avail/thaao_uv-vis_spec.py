...
def update_data_avail(instr):
    ...
    for i in date_list:
        filename = os.path.join(folder, 'thtc' + i.strftime('%y%m') + '.erv')
        if not os.path.exists(filename):
            uv_vis_spec_tmp = nasa_ames_parser_1010(filename)
            uv_vis_spec = uv_vis_spec.join(uv_vis_spec_tmp, how='outer', rsuffix='_duplicate')
            ...
            for elem in ['Julian day of the current year', 'Fractional julian day of the current year', 'year',
                         'day of month', 'hour', 'min', 'month number']:
                try:
...
def nasa_ames_parser_1010(fn):
    uv_vis_spec_tmp = pd.DataFrame()
    ...
    uv_vis_spec_tmp = uv_vis_spec_tmp.reset_index(drop=True)
    df = df.reset_index(drop=True)

    df.index = df.index + len(uv_vis_spec_tmp)

    return uv_vis_spec_tmp
